...
class Point:
    def __init__(self, a, b, system=SistemaCartesiano.CARTESIANO):
        if system == SistemaCartesiano.CARTESIANO:
            self.x = a
            self.y = b
        elif system == SistemaCartesiano.POLAR:
            self.x = a * cos(b)
            self.y = a * sin(b)
